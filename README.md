...
该工具适用于需要简单 Web 文件管理功能的场景，如本地开发辅助、小型服务器文件管理等。

# 核心功能概述
该软件实现了一个基于 Web 的文件管理系统，允许用户通过浏览器远程管理服务器上的文件和目录，支持文件上传、下载、创建、删除、复制、移动、重命名等基本操作，并具备断点续传、批量处理等增强功能同时支持多种文件格式的在线预览和播放。

## 🖥️ GUI图形界面
- 基于Tkinter的现代化图形用户界面
- 支持中文字体显示
- 可视化配置管理：工作目录、日志目录、端口设置
- 实时状态监控：服务器运行状态显示
- 日志窗口：实时查看操作日志
- 一键控制：启动/停止服务器、打开网站
